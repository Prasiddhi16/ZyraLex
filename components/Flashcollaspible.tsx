...
export const FlashCollapsible: React.FC<LevelCollapsibleProps> = ({
  level,
  locked,
}) => {
  ...
  useFocusEffect(
    useCallback(() => {
      const checkCompletion = async () => {
        // Check Flash Card level completion
        ...
        // Check each lesson completion
        const lessonStatus: Record<string, boolean> = {};

        await Promise.all(
          level.lessons.map(async (lesson) => {
            const value = await AsyncStorage.getItem(
              `flash_${level.levelId}_${lesson.lessonId}_completed`
            );
